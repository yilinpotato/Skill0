...
# and why:
#   - LoRA (rank/alpha) instead of full fine-tuning. 4k responses still need
#     conservative FSDP parameter/optimizer offload on two GPUs; those runtime
#     values are recorded below in run_config.env.
...
# actor lr=1e-6, save_freq=10, test_freq=5,
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-qwen3_4b_webshop_skill0_v3}"
...

{
  ...
  for key in TRAIN_DATA_SIZE GROUP_SIZE VAL_DATA_SIZE ENV_WORKER_CPUS RAY_NUM_CPUS ACTOR_LR TOTAL_TRAINING_STEPS SAVE_FREQ TEST_FREQ VAL_BEFORE_TRAIN RESUME_FROM_STEP LORA_RANK LORA_ALPHA INVALID_ACTION_PENALTY_COEF MAX_STEPS WEBSHOP_USE_SMALL WEBSHOP_HUMAN_GOALS MAX_PROMPT_LENGTH MAX_RESPONSE_LENGTH VLLM_GPU_MEMORY_UTILIZATION VLLM_MAX_NUM_BATCHED_TOKENS VLLM_MAX_NUM_SEQS VLLM_TP_SIZE PPO_MINI_BATCH_SIZE PPO_MICRO_BATCH_SIZE_PER_GPU LOG_PROB_MICRO_BATCH_PER_GPU REF_LOG_PROB_MICRO_BATCH_PER_GPU PARAM_OFFLOAD OPTIMIZER_OFFLOAD RESUME_DATALOADER_STATE GLOBAL_TOP_K_SCHEDULE GLOBAL_INTERNALIZATION_MODE SKILLZERO_INCLUDE_TASK_SPECIFIC ONPOLICY_HELPFULNESS_EVAL_ENABLED ONPOLICY_HELPFULNESS_AUTO_INTERNALIZE ONPOLICY_HELPFULNESS_DELTA_THRESHOLD ONPOLICY_HELPFULNESS_PATIENCE ONPOLICY_HELPFULNESS_REMOVE_PER_ROUND ONPOLICY_HELPFULNESS_MIN_ACTIVE_SKILLS; do
    echo "$key=${!key}"
  done
